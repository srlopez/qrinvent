...
class Mysql {
  ...
  Future<Map<String, Tuple2<String, String>>> readQRData(String qrCode) async {
    ...
    var SQLinvent = """
SELECT 'Nombre' AS Tipo, NAME AS Valor FROM hardware WHERE NAME = :qrcode
...
UNION SELECT 'S.O.' AS Tipo, hardware.osname AS Valor FROM hardware WHERE NAME = :qrcode
...
UNION SELECT 'Modelo' AS Tipo, b.smodel FROM hardware h INNER JOIN bios b ON h.id = b.hardware_id WHERE h.name = :qrcode
UNION SELECT 'Nº de serie' AS Tipo, b.ssn FROM hardware h INNER JOIN bios b ON h.id = b.hardware_id WHERE h.name = :qrcode
UNION SELECT 'BIOS - Versión' AS Tipo, b.bversion FROM hardware h INNER JOIN bios b ON h.id = b.hardware_id WHERE h.name = :qrcode
UNION SELECT 'BIOS - Fecha' AS Tipo, b.bdate FROM hardware h INNER JOIN bios b ON h.id = b.hardware_id WHERE h.name = :qrcode
...
UNION SELECT 'Memoria' AS Tipo, CONCAT(ROUND(hardware.memory/1024,0), " GB") AS Valor       FROM hardware WHERE NAME = :qrcode
...
UNION SELECT  s.name AS Tipo, CONCAT('Capacidad: ',CONCAT(ROUND(s.disksize / 1024, 0), " GB"), ' - Versión de firmware: ', s.firmware) AS Valor FROM hardware h
      INNER JOIN storages s ON h.id = s.hardware_id
      WHERE h.name = :qrcode
      AND (s.description = 'Disk drive' OR s.description = 'Unidad de disco')
UNION SELECT  d.letter AS Tipo,  CONCAT('Sistema de archivos: ', d.filesystem, ' - Libre: ', CONCAT(ROUND(d.free / 1024, 0), ' GB'), ' - Ocupado: ', CONCAT(ROUND(d.total / 1024, 0), ' GB')) AS Valor
      FROM hardware h
      INNER JOIN drives d ON h.id = d.hardware_id
      WHERE h.name = :qrcode
      AND d.type = 'Hard Drive'
UNION SELECT  c.name  AS Tipo, c.type AS Valor
      FROM hardware h
      INNER JOIN controllers c ON h.id = c.hardware_id
      WHERE h.name = :qrcode
UNION SELECT  c.description AS Tipo, CONCAT('MAC: ', c.macaddr, ' - Dirección IP: ', c.ipaddress, '/', c.ipmask, ' - Gateway: ', c.ipgateway, ' - Servidor DHCP: ', c.ipdhcp) AS Valor
      FROM hardware h
      INNER JOIN networks c ON h.id = c.hardware_id
      WHERE h.name = :qrcode AND c.status = 'Up'
UNION SELECT 'Modelo' AS Tipo, v.name AS Valor
      FROM hardware h INNER JOIN videos v ON h.id = v.hardware_id WHERE h.name = :qrcode
...
UNION SELECT s.name  AS Tipo, s.manufacturer AS Valor
      FROM hardware h
      INNER JOIN sounds s ON h.id = s.hardware_id
      WHERE h.name = :qrcode
UNION SELECT  monitors.manufacturer AS Tipo, CONCAT('Modelo: ', monitors.caption, ' - Número de serie: ', monitors.serial, ' - Tipo: ' , monitors.type, ' - Descripción: ' , monitors.description) AS Valor
      FROM
        hardware
        JOIN monitors ON hardware.id = monitors.hardware_id
      WHERE hardware.name = :qrcode
UNION SELECT
        c.name AS Tipo,
      ...
      WHERE h.name = :qrcode
UNION SELECT sn.name AS Tipo, sv.version AS Valor
      FROM
        hardware h
